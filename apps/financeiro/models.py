...
class Fatura(TenantModel):
    ...
    def esta_atrasada(self):
        return (
            self.status == "PENDENTE" and self.data_vencimento < timezone.now().date()
        )
...
class Lancamento(TenantModel):
    ...
    tipo = models.CharField(
        max_length=10, choices=[("ENTRADA", "Entrada"), ("SAIDA", "Saída")]
    )
    ...
    autorizado_por = models.ForeignKey(
        "auth.User", on_delete=models.PROTECT, related_name="autorizacoes_fin"
    )
    item_patrimonio = models.ForeignKey(
        "infraestrutura.ItemPatrimonio",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    )
    fatura_origem = models.ForeignKey(
        Fatura, on_delete=models.SET_NULL, null=True, blank=True
    )

    comprovante = models.FileField(
        upload_to="financeiro/comprovantes/", blank=True, null=True
    )
...
class FolhaPagamento(TenantModel):
    ...
    funcionario = models.ForeignKey(
        "auth.User", on_delete=models.CASCADE, related_name="folhas_pagamento"
    )
    ...
    impostos_encargos = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, help_text="INSS, FGTS, etc."
    )
    ...
    def salario_liquido(self):
        return (self.salario_base + self.bonus) - (
            self.descontos + self.impostos_encargos
        )
...
class Pagamento(models.Model):
    ...
    fatura = models.ForeignKey(
        Fatura, on_delete=models.CASCADE, related_name="pagamentos"
    )
    valor_pago = models.DecimalField(max_digits=10, decimal_places=2)
    data_pagamento = models.DateTimeField(default=timezone.now)
    metodo = models.CharField(max_length=20, choices=METODO_CHOICES)
    comprovante = models.FileField(
        upload_to="financeiro/pagamentos/", blank=True, null=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            cat_mensalidade, _ = CategoriaFinanceira.objects.get_or_create(
                nome="Mensalidade Aluno", tipo="RECEITA"
            )
        ...
        total_pago = (
            self.fatura.pagamentos.aggregate(total=models.Sum("valor_pago"))["total"]
            or 0
        )
