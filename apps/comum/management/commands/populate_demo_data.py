...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--clear", action="store_true", help="Limpa o banco antes de popular"
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["clear"]:
            ...
            # Ordem correta para evitar erros de integridade referencial
        ...
        # --- INFRAESTRUTURA ---
        unidade, _ = UnidadeEscolar.objects.get_or_create(
            nome="Campus Central", eh_sede=True
        )
        cat_moveis, _ = CategoriaBem.objects.get_or_create(
            nome="Mobiliário", descricao="Mesas, cadeiras, armários"
        )
        cat_ti, _ = CategoriaBem.objects.get_or_create(
            nome="Tecnologia", descricao="Computadores, periféricos"
        )

        ItemPatrimonio.objects.get_or_create(
            tombamento="PAT-2024-001",
            nome="MacBook Pro M3 - Lab 1",
            categoria=cat_ti,
            unidade=unidade,
            estado_conservacao="NOVO",
        )
        ItemPatrimonio.objects.get_or_create(
            tombamento="PAT-2024-002",
        ...
        ItemPatrimonio.objects.get_or_create(
            tombamento="PAT-2024-003",
            nome="Mesa Diretor - Madeira Maciça",
            categoria=cat_moveis,
            unidade=unidade,
            estado_conservacao="BOM",
        )

        item_papel, _ = ItemEstoque.objects.get_or_create(
            nome="Resma Papel A4", unidade_medida="Pacote", estoque_minimo=10
        )
        SaldoEstoque.objects.get_or_create(
            item=item_papel, unidade=unidade, quantidade=50
        )
        ...
        for t, a, i, q in livros_data:
            Livro.objects.get_or_create(
                titulo=t, autor=a, isbn=i, quantidade_total=q
            )
        ...
        # --- RESPONSÁVEIS ---
        responsaveis_info = [
            ("Sr. José Silva", "jose.pai", "666.666.666-66"),
            ("Sra. Maria Santos", "maria.mae", "777.777.777-77"),
            ("Sr. Antônio Souza", "antonio.pai", "888.888.888-88"),
        ]
        responsaveis = []
        for nome, username, cpf in responsaveis_info:
            u, created = User.objects.get_or_create(
            ...
            r, _ = Responsavel.objects.get_or_create(
                ...
                nome_completo=nome, 
                cpf=cpf,
                controle_parental_ativo=random.choice([True, False])
            )
        ...
        for t_nome, t_turno, t_ano in turmas_data:
            turma, _ = Turma.objects.get_or_create(
                nome=t_nome, turno=t_turno, ano=t_ano
            )
            ...
            for i in range(1, 11):
                ...
                aluno, created_al = Aluno.objects.get_or_create(
                    user=u, nome_completo=al_nome, turma=turma, cpf=al_cpf
                )
                
                # O save() do Aluno agora sincroniza automaticamente o username com a matrícula
                if created_al:
                    self.stdout.write(f"  - Criado: {al_nome} (Matrícula: {aluno.matricula})")

                # Vincula a um responsável aleatório
                resp = random.choice(responsaveis)
                resp.alunos.add(aluno)

                # Ficha de Saúde para metade dos alunos
                if i % 2 == 0:
                    ficha, _ = FichaMedica.objects.get_or_create(
                        aluno=aluno,
                        tipo_sanguineo="O+",
                        alergias="Lactose" if i % 4 == 0 else "",
                    )
                    RegistroVacina.objects.create(
                        ficha=ficha,
                        nome_vacina="Gripe",
                        data_dose=timezone.now().date() - timedelta(days=60),
                    )
            ...
            for d_nome in disciplinas_nomes:
                ...
                disc, _ = Disciplina.objects.get_or_create(
                    nome=d_nome, turma=turma, professor=prof
                )
        ...
        # --- SINCRONIZAÇÃO FINAL ---
        self.stdout.write("Sincronizando logins com matrículas...")
        for aluno in Aluno.objects.all():
            if aluno.matricula and aluno.user.username != aluno.matricula:
                aluno.user.username = aluno.matricula
                aluno.user.save()

        self.stdout.write(self.style.SUCCESS(f"População finalizada com sucesso!"))
