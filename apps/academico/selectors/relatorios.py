...
def get_metricas_gerais(ano=None, mes=None):
    ...
    metricas = {
        ...
        "alunos_transferidos": Aluno.objects.filter(
    ...
    if metricas["total_alunos"] > 0:
        metricas["taxa_evasao"] = (
            metricas["alunos_evadidos"] / metricas["total_alunos"]
        ) * 100
    else:
    ...
    metricas["concluintes"] = Aluno.objects.filter(
        Q(turma__nome__icontains="9") | Q(turma__nome__icontains="3º"), turma__ano=ano
    ).count()
...
def get_performance_academica(ano):
    ...
    perf_data = (
        Aluno.objects.filter(turma__ano=ano)
        .annotate(media_aluno=Avg(calc_media))
        .aggregate(
            aprovados=Count(
                Case(
                    When(media_aluno__gte=6.0, then=Value(1)),
                    output_field=IntegerField(),
                )
            ),
            conselho=Count(
                Case(
                    When(media_aluno__range=(4.9, 5.9), then=Value(1)),
                    output_field=IntegerField(),
                )
            ),
            reprovados=Count(
                Case(
                    When(media_aluno__lt=4.9, then=Value(1)),
                    output_field=IntegerField(),
                )
            ),
        )
...
def get_dados_historico(aluno_id):
    ...
    try:
        ...
        notas = (
            Nota.objects.filter(aluno=aluno)
            .select_related("disciplina", "disciplina__turma")
            .order_by("disciplina__turma__ano", "disciplina__nome")
        )
        ...
        freq_map = {
            f["disciplina"]: (
                f["presencas"] / f["total"] * 100 if f["total"] > 0 else 100
            )
        ...
        return {"aluno": aluno, "historico": historico}
