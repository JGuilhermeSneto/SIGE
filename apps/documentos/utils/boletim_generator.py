...
class BoletimGenerator(InstitutionalPDF):
    ...
    def generate(self):
        ...
        elements.append(
            Paragraph(
                "BOLETIM DE DESEMPENHO ESCOLAR", self.styles["InstitutionalTitle"]
            )
        )
        ...
        info_data = [
            [
                Paragraph("ALUNO", self.styles["LabelStyle"]),
                Paragraph("TURMA", self.styles["LabelStyle"]),
            ],
            [
                Paragraph(self.aluno.nome_completo.upper(), self.styles["ValueStyle"]),
                Paragraph(self.aluno.turma.nome, self.styles["ValueStyle"]),
            ],
            [
                Paragraph("CPF", self.styles["LabelStyle"]),
                Paragraph("ANO LETIVO", self.styles["LabelStyle"]),
            ],
            [
                Paragraph(self.aluno.cpf, self.styles["ValueStyle"]),
                Paragraph(str(self.aluno.turma.ano), self.styles["ValueStyle"]),
            ],
        ]

        info_table = Table(
            info_data, colWidths=[10 * 10, 8 * 10]
        )  # cm to points approximately or just use widths
        ...
        for i, item in enumerate(self.dados):
            media_val = (
                float(item["nota"].media)
                if item["nota"] and item["nota"].media is not None
                else 0
            )
            if media_val < 7.0:
                style.add(
                    "TEXTCOLOR", (5, i + 1), (5, i + 1), colors.HexColor("#dc2626")
                )  # Vermelho
            else:
                style.add(
                    "TEXTCOLOR", (5, i + 1), (5, i + 1), colors.HexColor("#16a34a")
                )  # Verde
