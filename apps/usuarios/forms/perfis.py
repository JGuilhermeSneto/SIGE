...
class ProfessorForm(BaseModelForm):
    ...
    senha = forms.CharField(required=False, widget=forms.PasswordInput(), help_text="Se vazio, usa os 6 primeiros dígitos do CPF.")
    senha_confirmacao = forms.CharField(required=False, widget=forms.PasswordInput(), label="Confirmar Senha")
    ...
    def save(self, commit=True):
        with transaction.atomic():
            ...
            if prof.pk and getattr(prof, "user", None):
                ...
                user, created = User.objects.get_or_create(username=un, defaults={'email': email})
                if not created:
                    user.email = email
