...
def visualizar_saude_aluno(request, aluno_id):
    ...
    if hasattr(request.user, "professor") and hasattr(aluno, "turma") and aluno.turma:
        eh_professor = aluno.turma.disciplinas.filter(
            professor=request.user.professor
        ).exists()
    eh_proprio_aluno = (
        hasattr(request.user, "aluno") and request.user.aluno.id == aluno.id
    )

    if not (eh_gestor or eh_professor or eh_proprio_aluno):
        messages.error(
            request, "Você não tem permissão para ver dados de saúde deste aluno."
        )
...
def editar_ficha_medica(request, aluno_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, f"Ficha médica de {aluno.nome_completo} atualizada!"
            )
...
def enviar_atestado(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, "Atestado enviado com sucesso! Aguarde a análise da gestão."
            )
...
def gestao_atestados(request):
    ...
    atestados = AtestadoMedico.objects.filter(status=status_filtro).select_related(
        "usuario"
    )

    return render(
        request,
        "saude/gestao_atestados.html",
        {
            "atestados": atestados,
...
def revisar_atestado(request, atestado_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            atestado.analisado_por = (
                request.user.gestor if hasattr(request.user, "gestor") else None
            )
            atestado.save()
            ...
            if atestado.status == "APROVADO":
                ...
                if hasattr(atestado.usuario, "aluno"):
                    ...
                    messages.success(
                        request,
                        f"Atestado aprovado! {total_justificadas} faltas do aluno {aluno.nome_completo} foram justificadas.",
                    )
                ...
                # --- LÓGICA PARA PROFESSORES ---
                    ...
                    # Grades do professor
                    grades = GradeHorario.objects.filter(
                    ...
                    messages.success(
                        request,
                        f"Atestado aprovado! {total_suspendas} sessões de aula do professor {professor.nome_completo} foram marcadas como suspensas.",
                    )
