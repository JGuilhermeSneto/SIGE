...
class RegistrosViewsTest(TestCase):
    ...
    def test_cadastrar_professor_post(self):
        ...
        self.assertTrue(Professor.objects.filter(nome_completo="Novo Professor").exists())
    ...
    def test_reativar_usuario(self):
        other_user = User.objects.create_user(username="inactive", password=self.password, is_active=False)
    ...
    def test_excluir_professor(self):
        prof_user = User.objects.create_user(username="prof_del", password=self.password)
        prof = Professor.objects.create(user=prof_user, nome_completo="Del Prof", cpf="999.999.999-99")
    ...
    def test_excluir_aluno(self):
        aluno_user = User.objects.create_user(username="aluno_del", password=self.password)
        aluno = Aluno.objects.create(user=aluno_user, nome_completo="Del Aluno", cpf="888.888.888-88")
    ...
    def test_excluir_gestor(self):
        gestor_user = User.objects.create_user(username="gestor_del", password=self.password)
        gestor = Gestor.objects.create(user=gestor_user, nome_completo="Del Gestor", cpf="777.777.777-77")
    ...
    def test_desativar_usuario_proprio(self):
        response = self.client.post(reverse('desativar_usuario', args=[self.super_user.id]))
        ...
        self.assertTrue(any("Você não pode desativar seu próprio acesso!" in m.message for m in msgs))
