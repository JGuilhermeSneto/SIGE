    ...
    def clean(self):
        ...
        if qs.exists():
            raise forms.ValidationError(
                "JÃ¡ existe uma turma com este nome para o ano informado."
            )
...
class DisciplinaForm(BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["professor"].queryset = Professor.objects.select_related(
            "user"
        ).all()
    ...
    def clean(self):
        ...
        if nome and turma:
            ...
            if qs.exists():
                raise forms.ValidationError(
                    f"A disciplina '{nome}' jÃ¡ estÃ¡ cadastrada para esta turma."
                )
...
class AtividadeProfessorForm(BaseModelForm):
    ...
    def clean(self):
        ...
        if tipo != "PROVA" and not prazo_final:
            self.add_error(
                "prazo_final",
                "Para atividades e trabalhos, é obrigatório definir um prazo final de entrega.",
            )
        ...
        if data and tipo == "PROVA":
            ...
            if not evento:
                self.add_error(
                    "data",
                    "Provas só podem ser cadastradas em datas marcadas como 'Semana de Prova' no Calendário Escolar Oficial.",
                )
        return cleaned_data
