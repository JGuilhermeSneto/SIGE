...
class BibliotecaViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def reservar(self, request, pk=None):
        ...
        if not hasattr(user, "aluno"):
            return Response(
                {"detail": "Apenas alunos podem fazer reservas via web."},
        ...
        if livro.exemplares_disponiveis <= 0:
            return Response(
                {"detail": "Não há exemplares físicos disponíveis para reserva."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        # Regra 2: Validar o limite de 2 livros simultâneos
        livros_ativos = Emprestimo.objects.filter(
            usuario_aluno=aluno, status__in=["ATIVO", "RESERVA"]
        ).count()

        if livros_ativos >= 2:
            return Response(
                {
                    "detail": "Você já atingiu o limite máximo de 2 livros (entre reservas e ativos)."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        reserva_dupla = Emprestimo.objects.filter(
            usuario_aluno=aluno, livro=livro, status__in=["ATIVO", "RESERVA"]
        ).exists()

        if reserva_dupla:
            return Response(
                {"detail": "Você já possui uma reserva ou empréstimo deste livro."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            {"detail": "Reserva efetuada com sucesso!", "emprestimo_id": emprestimo.id},
...
class MeusEmprestimosViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if hasattr(user, "aluno"):
            return Emprestimo.objects.filter(usuario_aluno=user.aluno).order_by(
                "-data_emprestimo"
            )
