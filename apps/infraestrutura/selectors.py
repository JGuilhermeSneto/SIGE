...
class InfraSelector:
    ...
    def get_painel_data(query_text=None):
        ...
        patrimonios = ItemPatrimonio.objects.all().select_related(
            "categoria", "unidade", "responsavel", "ambiente"
        )
        saldos_estoque = (
            SaldoEstoque.objects.all()
            .select_related("item", "unidade")
            .order_by("item__nome")
        )
        ...
        metricas = {
            ...
            "valor_investido": patrimonios.aggregate(total=Sum("valor_aquisicao"))[
                "total"
            ]
            or 0,
            "manutencoes_pendentes": ManutencaoBem.objects.filter(
                data_realizacao__isnull=True
            ).count(),
            "total_ambientes": Ambiente.objects.count(),
            "itens_criticos": saldos_estoque.filter(
                quantidade__lte=F("item__estoque_minimo")
            ).count(),
            "estado_precario": patrimonios.filter(
                estado_conservacao__in=["DANIFICADO", "INSERVIVEL"]
            ).count(),
        }
        ...
        # Gráfico: Categorias
        categorias_qs = (
            ItemPatrimonio.objects.values("categoria__nome")
