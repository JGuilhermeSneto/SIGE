...
def get_distribuicao_notas(ano):
    ...
    calc_media = ExpressionWrapper(
        (
            Coalesce(F("nota1"), Value(0.0))
            + Coalesce(F("nota2"), Value(0.0))
            + Coalesce(F("nota3"), Value(0.0))
            + Coalesce(F("nota4"), Value(0.0))
        )
        / 4.0,
    ...
    stats = (
        Nota.objects.filter(disciplina__turma__ano=ano)
        .annotate(media_nota=calc_media)
        .aggregate(
...
def get_materias_criticas(ano):
    ...
    return (
        Disciplina.objects.filter(turma__ano=ano)
        .annotate(media_geral=Avg(calc_media))
...
def get_engajamento_atividades(ano):
    ...
    return (
        round((total_entregue / total_esperado * 100), 1) if total_esperado > 0 else 0
    )
