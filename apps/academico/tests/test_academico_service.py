...
class AcademicoServiceTest(TestCase):
    ...
    def test_criar_turma_com_disciplinas(self):
        ...
        self.assertEqual(
            Disciplina.objects.get(nome="Matemática").professor, self.professor
        )
    ...
    def test_calcular_situacao_aluno(self):
        # Aprovado
        self.assertEqual(
            AcademicoService.calcular_situacao_aluno(7.5, 80)["texto"], "Aprovado"
        )
        # Recuperação
        self.assertEqual(
            AcademicoService.calcular_situacao_aluno(6.0, 80)["texto"], "Recuperação"
        )
        # Reprovado por média
        self.assertEqual(
            AcademicoService.calcular_situacao_aluno(4.0, 80)["texto"], "Reprovado"
        )
        # Reprovado por falta
        self.assertEqual(
            AcademicoService.calcular_situacao_aluno(9.0, 70)["texto"],
            "Reprovado por Falta",
        )
