...
# =============================================================
# _get_grade_formatada  (linhas 94, 98-105)
# =============================================================

class GetGradeFormatadasTest(TestCase):

    def _call(self, valor):
        from core.views import _get_grade_formatada
        return _get_grade_formatada(valor)
...
class GetOcupadosPorProfessorTest(TestCase):
    ...
    @patch("core.views.Turma")
    def test_sem_turmas_retorna_set_vazio(self, MockTurma):
        MockTurma.objects.filter.return_value.exclude.return_value = []
        MockTurma.objects.filter.return_value = []
        resultado = self._call(1, 2024)
        self.assertEqual(resultado, set())

    @patch("core.views.Turma")
    def test_turma_sem_grade_horaria_ignorada(self, MockTurma):
        turma = MagicMock()
        turma.grade_horaria = None
        MockTurma.objects.filter.return_value = [turma]
        resultado = self._call(1, 2024)
        self.assertEqual(resultado, set())

    @patch("core.views.Turma")
    def test_grade_como_dict_retorna_slots_do_professor(self, MockTurma):
        turma = MagicMock()
        turma.grade_horaria = {
            "segunda": {
                "0": {"professor_id": "5"},
        ...
        self.assertNotIn("segunda-1", resultado)

    @patch("core.views.Turma")
    def test_grade_como_string_json_e_parseada(self, MockTurma):
        import json
        turma = MagicMock()
        turma.grade_horaria = json.dumps({
            "terca": {"0": {"professor_id": "3"}}
        })
        MockTurma.objects.filter.return_value = [turma]
        resultado = self._call(3, 2024)
        self.assertIn("terca-0", resultado)

    @patch("core.views.Turma")
    def test_grade_json_invalido_e_ignorado(self, MockTurma):
        turma = MagicMock()
        turma.grade_horaria = "json_invalido{{{"
        MockTurma.objects.filter.return_value = [turma]
        resultado = self._call(1, 2024)
        self.assertEqual(resultado, set())

    @patch("core.views.Turma")
    def test_grade_nao_dict_apos_parse_ignorada(self, MockTurma):
        import json
        turma = MagicMock()
        turma.grade_horaria = json.dumps([1, 2, 3])  # lista, não dict
        MockTurma.objects.filter.return_value = [turma]
        resultado = self._call(1, 2024)
        self.assertEqual(resultado, set())

    @patch("core.views.Turma")
    def test_turma_atual_excluida_dos_conflitos(self, MockTurma):
        turma = MagicMock()
        turma.grade_horaria = {"segunda": {"0": {"professor_id": "1"}}}
        qs_mock = MagicMock()
        qs_mock.exclude.return_value = []
        MockTurma.objects.filter.return_value = qs_mock
        resultado = self._call(1, 2024, turma_id_atual=42)
        self.assertEqual(resultado, set())

    @patch("core.views.Turma")
    def test_info_nao_dict_no_slot_ignorada(self, MockTurma):
        turma = MagicMock()
        turma.grade_horaria = {"segunda": {"0": "string_invalida"}}
        MockTurma.objects.filter.return_value = [turma]
        resultado = self._call(1, 2024)
        self.assertEqual(resultado, set())
