...
class BibliotecaViewsTest(TestCase):
    def setUp(self):
        ...
        # Aluno
        self.aluno_user = User.objects.create_user(username="aluno", password=self.password)
        self.turma = Turma.objects.create(nome="1A", turno="manha", ano=2024)
        self.aluno = Aluno.objects.create(user=self.aluno_user, nome_completo="Aluno 1", cpf="1", data_nascimento="2010-01-01", turma=self.turma)
        
        # Livro
        self.livro = Livro.objects.create(titulo="Dom Casmurro", autor="Machado de Assis", quantidade_total=5)
    ...
    def test_reservar_livro(self):
        ...
        self.assertEqual(Emprestimo.objects.filter(livro=self.livro, usuario_aluno=self.aluno).count(), 1)
    ...
    def test_novo_emprestimo(self):
        ...
        data = {
            ...
            'data_devolucao_prevista': (timezone.now() + timezone.timedelta(days=14)).date(),
            'status': 'ATIVO'
        }
