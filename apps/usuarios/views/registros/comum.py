...
def reativar_usuario(request, user_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f"Usuário {user_to_activate.username} reativado com sucesso!"
        )
...
def desativar_usuario(request, user_id):
    ...
    if request.method == "POST":
        if user_to_deactivate == request.user:
            ...
            messages.success(
                request,
                f"Usuário {user_to_deactivate.username} desativado com sucesso!",
            )
