            ...
            for i in range(1, 5):
                ...
                if valor == "":
                    ...
                    try:
                        ...
                        if 0 <= v <= 10:
                            setattr(nota_obj, f"nota{i}", v)
                    except ValueError:
                        messages.error(
                            request, f"Erro na nota {i} de {aluno.nome_completo}"
                        )
            nota_obj.save()
...
def lancar_chamada(request, disciplina_id):
    ...
    try:
        data_sel = date.fromisoformat(data_str) if data_str else timezone.now().date()
    except (ValueError, TypeError):
        data_sel = timezone.now().date()

    alunos = (
        disciplina.turma.alunos.all()
        .select_related("user", "ficha_medica")
        .order_by("nome_completo")
    )
    freq_obs = {
        f.aluno_id: f
        for f in Frequencia.objects.filter(disciplina=disciplina, data=data_sel)
    }

    # IDs de usuários com atestado aprovado para este dia
    ids_usuarios_de_atestado = AtestadoMedico.objects.filter(
        status="APROVADO", data_inicio__lte=data_sel, data_fim__gte=data_sel
    ...
    if request.method == "POST":
        ...
        for aluno in alunos:
            ...
            obs = (
                request.POST.get(f"observacao_{aluno.id}", "").strip()
                if not pres
                else ""
            )
            ...
            detalhe = (
                "Você foi marcado como presente."
                if pres
                else "Você foi marcado com falta."
            )
            NotificacaoServico.criar(
                user=aluno.user,
                tipo="CHAMADA",
        ...
        return redirect(
            f"{reverse('lancar_chamada', args=[disciplina.id])}?data={data_sel}"
        )
    ...
    return render(
        ...
        "frequencia/lancar_chamada.html",
        {
            "disciplina": disciplina,
            "alunos": alunos,
            "data_selecionada": data_sel.isoformat(),
...
def historico_frequencia(request, disciplina_id):
    ...
    disciplina = get_object_or_404(Disciplina, id=disciplina_id)
    if not (
        is_super_ou_gestor(request.user)
        or (
            hasattr(request.user, "professor")
            and disciplina.professor == request.user.professor
        )
        ...
        detalhes.append(
            {
                ...
                "faltas": f["total"] - f["presencas"],
                "percentual_faltas": (
                    round(((f["total"] - f["presencas"]) / f["total"] * 100), 1)
                    if f["total"] > 0
                    else 0
                ),
            }
