...
class RelatorioMasterPDF:
    ...
    def desenhar_cabecalho(self):
        ...
        self.canvas.drawString(
            1 * cm, self.height - 1.2 * cm, f"S.I.G.E | {self.titulo}"
        )

        self.canvas.setFont("Helvetica", 10)
        data_hora = datetime.datetime.now().strftime("%d/%m/%Y %H:%M")
        self.canvas.drawRightString(
            self.width - 1 * cm, self.height - 1.2 * cm, f"Gerado em: {data_hora}"
        )
    ...
    def adicionar_tabela_evasao(self, eixo_y, dataset_alunos):
        ...
        self.canvas.drawString(
            1 * cm, eixo_y, "LISTA CRÍTICA (Frequência < 75% ou Média < 5.0)"
        )
        ...
        self.canvas.setFillColor(colors.white)
        self.canvas.rect(
            1 * cm, eixo_y, self.width - 2 * cm, 0.8 * cm, fill=1, stroke=0
        )
        ...
        for aluno in dataset_alunos[
            :20
        ]:  # Protege para renderizar no max 20 pra caber 1 pag no doc base
            freq = f"{aluno.taxa_freq:.2f}%" if aluno.taxa_freq else "N/A"
            media = f"{aluno.avg_nota:.2f}" if aluno.avg_nota else "N/A"

            self.canvas.drawString(
                1.2 * cm, eixo_y + 0.25 * cm, aluno.nome_completo[:25] + "..."
            )
            self.canvas.drawString(
                8.0 * cm, eixo_y + 0.25 * cm, str(aluno.turma.nome)[:15]
            )
    ...
    def gerar_pdf(self, metricas):
        ...
        self.desenhar_bloco_estatistica(
            1 * cm, y_cards, "Geral (Alunos)", metricas["total_alunos"]
        )
        self.desenhar_bloco_estatistica(
            6.5 * cm, y_cards, "Em Risco Critico", metricas["total_risco"], alerta=True
        )
        self.desenhar_bloco_estatistica(
            12 * cm, y_cards, "Livros Emprestados", metricas["livros_ativos"]
        )
