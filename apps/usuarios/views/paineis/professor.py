...
def painel_professor(request):
    ...
    total_possiveis, total_lancadas, total_alunos_unicos, _ = _acumular_notas_professor(
        disciplinas
    )
    ...
    contexto["meus_emprestimos"] = (
        Emprestimo.objects.filter(usuario_professor=professor)
        .exclude(status="DEVOLVIDO")
        .select_related("livro")
        .order_by("-data_emprestimo")
    )

    contexto["livros_devolvidos"] = (
        Emprestimo.objects.filter(usuario_professor=professor, status="DEVOLVIDO")
        .select_related("livro")
