...
def dashboard_saude_inclusao(request):
    ...
    fichas_com_alergia = FichaMedica.objects.exclude(alergias__isnull=True).exclude(
        alergias=""
    )
    ...
    sanguineo_qs = FichaMedica.objects.values("tipo_sanguineo").annotate(
        total=Count("id")
    )
    ...
    contexto = {
        "stats": {
            ...
            "percentual_pcd": (
                round((total_pcd / total_alunos * 100), 1) if total_alunos > 0 else 0
            ),
        },
...
def central_inteligencia(request):
    ...
    calc_media_disc = ExpressionWrapper(
        (
            ...
            + Coalesce(F("alunos__notas__nota4"), Value(0.0))
        )
        / 4.0,
    ...
    dados_turmas = (
        Turma.objects.filter(ano=ano_filtro)
        .annotate(
            ...
            taxa_frequencia=Case(
                When(
                    ...
                    * Count(
                        "alunos__frequencias",
                        filter=Q(alunos__frequencias__presente=True),
                    )
                    / F("total_freq"),
                ),
                default=Value(100.0),
    ...
    calc_media_aluno = ExpressionWrapper(
        (
            ...
            + Coalesce(F("notas__nota4"), Value(0.0))
        )
        / 4.0,
        output_field=FloatField(),
    )
    alunos_qs = Aluno.objects.annotate(
        ...
        taxa_freq=Case(
            When(
                tot_freq__gt=0,
                then=100.0
                * Count("frequencias", filter=Q(frequencias__presente=True))
                / F("tot_freq"),
            ),
            default=Value(100.0),
            output_field=FloatField(),
    ...
    alunos_em_risco = alunos_qs.filter(
        Q(taxa_freq__lt=75) | (Q(avg_nota__lt=5.0) & Q(avg_nota__gt=0))
    )
    ...
    status_dict = {
        row["status_matricula"]: row["total"]
        for row in status_qs
        if row["status_matricula"]
    }
    ...
    for al in alunos:
        writer.writerow(
            [
                al.nome_completo,
                al.turma.nome,
                al.turma.get_turno_display(),
                "(XX) 9XXXX-XXXX",
                f"{al.taxa_freq:.2f}%" if al.taxa_freq else "0%",
                f"{al.avg_nota:.2f}" if al.avg_nota else "0.00",
            ]
        )
    return response
