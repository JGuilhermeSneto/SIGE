...
def meus_planejamentos(request):
    ...
    if disciplina_id:
        try:
            ...
            contexto["grade_ativa"] = GradeHorario.objects.filter(
                disciplina=disc_obj
...
def salvar_planejamento(request):
    ...
    if request.method == "POST":
        ...
        if not (data_str and grade_id and conteudo and disciplina_id):
            return JsonResponse(
                {"erro": "Preencha todos os campos antes de salvar."}, status=400
            )
        ...
        try:
            ...
            GradeHorario.DoesNotExist,
        ):
            return JsonResponse(
                {"erro": "Parâmetros inválidos ou não encontrados."}, status=400
            )
        ...
        if grade.dia != dia_str:
            ...
            return JsonResponse(
                {
                    "erro": f"Operação bloqueada: A data escolhida ({data_str}) é {dia_str.title()}, mas o slot selecionado é de {grade.get_dia_display()}."
                },
                status=400,
            )
