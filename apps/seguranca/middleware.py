...
from apps.seguranca.models import LogAuditoria, BlacklistIP, ConfiguracaoSeguranca, LogErro
    ...
    def process_request(self, request):
        ...
        if path.startswith('/admin/') and not request.user.is_authenticated:
            ...
            if count >= 3:
                ...
                logger.warning(f"SECURITY: IP {ip} banido automaticamente por tentativa de invasão no Admin.")
...
class ManutencaoMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        path = request.path
        if any(p in path for p in ['/static/', '/media/', '/admin/', '/seguranca/', '/login/', '/logout/']):
            return None
        ...
        if config.manutencao_ativa:
            ...
            if request.user.is_authenticated:
                if request.user.is_superuser:
                    pode_acessar = True
                elif config.permite_login_gestor and hasattr(request.user, 'perfil') and request.user.perfil == 'gestor':
                    pode_acessar = True
            
            if not pode_acessar:
                return render(request, 'seguranca/manutencao.html', {
...
class AuditMiddleware(MiddlewareMixin):
    ...
    SENSITIVE_PATHS = ['/saude/', '/financeiro/', '/infraestrutura/', '/admin/', '/seguranca/', '/usuarios/perfil/']
