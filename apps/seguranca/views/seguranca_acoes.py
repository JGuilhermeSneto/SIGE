...
def bloquear_ip(request, ip):
    ...
    BlacklistIP.objects.get_or_create(ip_endereco=ip, defaults={'motivo': motivo, 'bloqueado_por': request.user})
...
def bloquear_usuario(request, user_id):
    ...
    messages.warning(request, f"O usuário {user.username} foi bloqueado e todas as suas sessões foram encerradas.")
    return redirect('seguranca:dashboard')
...
def toggle_manutencao(request):
    ...
    if request.method == 'POST':
        ...
        NotificacaoServico.notificar_grupo(
            ...
            mensagem=f"O sistema entrou em modo de manutenção e o acesso externo está restrito." if config.manutencao_ativa else "O sistema está operando normalmente agora.",
...
def honeypot_view(request):
    ...
    logger.warning(f"HONEYPOT: IP {ip} bloqueado automaticamente ao tentar acessar {path}")
    return HttpResponseForbidden("Acesso Proibido. Seu IP foi permanentemente bloqueado por atividade suspeita.")
...
def reportar_bug(request):
    ...
    if request.method == 'POST':
        ...
        if screenshot and not validar_assinatura_arquivo(screenshot):
            messages.error(request, "O arquivo de evidência parece ser inválido ou malicioso.")
        ...
        messages.success(request, "Obrigado! Seu relatório de bug foi enviado para nossa equipe de segurança.")
        return redirect(url_origem or 'dashboard')
