...
class AcademicoViewsTest(TestCase):
    def setUp(self):
        ...
        self.professor = Professor.objects.create(
            user=self.prof_user, nome_completo="Prof Teste", cpf="111.111.111-11", data_nascimento="1980-01-01"
        )
        
        # Turma e Disciplina
        self.turma = Turma.objects.create(nome="1A", turno="manha", ano=timezone.now().year)
        ...
        self.aluno = Aluno.objects.create(
            user=self.aluno_user, nome_completo="Aluno Teste", cpf="222.222.222-22", 
            data_nascimento="2010-01-01", turma=self.turma
        )
    ...
    def test_cadastrar_atividade_get(self):
        self.client.login(username="prof", password=self.password)
        response = self.client.get(reverse('cadastrar_atividade', args=[self.disciplina.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_cadastrar_disciplina_post(self):
        ...
        response = self.client.post(reverse('cadastrar_disciplina_para_turma', args=[self.turma.id]), data)
        self.assertTrue(Disciplina.objects.filter(nome="Física").exists())
        self.assertRedirects(response, reverse('listar_disciplinas_turma', args=[self.turma.id]))
    ...
    def test_cadastrar_atividade_post(self):
        ...
        response = self.client.post(reverse('cadastrar_atividade', args=[self.disciplina.id]), data)
        self.assertTrue(AtividadeProfessor.objects.filter(titulo="Novo Trabalho").exists())
        self.assertRedirects(response, reverse('listar_atividades', args=[self.disciplina.id]))
    ...
    def test_lancar_notas_atividade_post(self):
        ...
        atividade = AtividadeProfessor.objects.create(
            titulo="Atividade com Nota", tipo="ATIVIDADE", data="2024-05-10", 
            prazo_final=timezone.now(), disciplina=self.disciplina
        )
        data = {
            f"nota_{self.aluno.id}": "8.5",
            f"obs_{self.aluno.id}": "Bom trabalho"
        }
        response = self.client.post(reverse('lancar_notas_atividade', args=[self.disciplina.id, atividade.id]), data)
        self.assertEqual(response.status_code, 302)
        
        from apps.academico.models.desempenho import NotaAtividade
        self.assertTrue(NotaAtividade.objects.filter(aluno=self.aluno, atividade=atividade, valor=8.5).exists())

    def test_visualizar_grade_professor_get(self):
        self.client.login(username="prof", password=self.password)
        response = self.client.get(reverse('visualizar_grade_professor', args=[self.turma.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_excluir_disciplina(self):
        self.client.login(username="admin", password=self.password)
        response = self.client.post(reverse('excluir_disciplina', args=[self.disciplina.id]))
    ...
    def test_marcar_notificacao_lida(self):
        ...
        notif = Notificacao.objects.create(usuario=self.aluno_user, titulo="Teste", mensagem="Msg")
        self.client.login(username="aluno", password=self.password)
    ...
    def test_marcar_todas_notificacoes_lidas(self):
        ...
        self.assertFalse(Notificacao.objects.filter(usuario=self.aluno_user, lida=False).exists())

    def test_controlar_liberacao_gabarito_sim(self):
        atividade = AtividadeProfessor.objects.create(
            titulo="Ativ Gabarito", tipo="ATIVIDADE", data="2024-05-10", 
            prazo_final=timezone.now(), disciplina=self.disciplina
        )
        # Adiciona uma questão para poder liberar gabarito
        from apps.academico.models.academico import Questao
        Questao.objects.create(atividade=atividade, enunciado="Q1", tipo="MULTIPLA_ESCOLHA")
        
        self.client.login(username="prof", password=self.password)
        response = self.client.post(reverse('controlar_liberacao_gabarito', args=[self.disciplina.id, atividade.id]), {"decisao": "sim"})
