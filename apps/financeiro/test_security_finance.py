...
class FinanceSecurityTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="finuser", password="Password123!"
        )
    ...
    def test_fatura_encryption(self):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT link_pagamento FROM fin_fatura WHERE id = %s", [self.fatura.id]
            )
            row = cursor.fetchone()
        ...
        self.assertEqual(
            fatura_db.link_pagamento, "https://pagamento.seguro.com/token123"
        )
