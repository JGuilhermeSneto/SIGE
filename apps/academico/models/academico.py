...
class GradeHorario(models.Model):
    ...
    disciplina = models.ForeignKey(
        Disciplina, on_delete=models.CASCADE, related_name="grades"
    )
...
class AtividadeProfessor(models.Model):
    ...
    disciplina = models.ForeignKey(
        ...
        help_text="Disciplina associada",
    )
    ...
    data = models.DateField(
        help_text="Data do agendamento (para provas, deve coincidir com o calendário)"
    )
    prazo_final = models.DateTimeField(
        null=True,
        blank=True,
        help_text="Data e Hora limite para entrega (Obrigatório para Atividade/Trabalho)",
    )
...
class Questao(models.Model):
    ...
    TIPO_QUESTAO = [
        ...
        ("DISCURSIVA", "Discursiva (Texto)"),
    ]
    atividade = models.ForeignKey(
        AtividadeProfessor, on_delete=models.CASCADE, related_name="questoes"
    )
...
class EntregaAtividade(models.Model):
    ...
    aluno = models.ForeignKey(
        "usuarios.Aluno", on_delete=models.CASCADE, related_name="entregas"
    )
    atividade = models.ForeignKey(
        AtividadeProfessor, on_delete=models.CASCADE, related_name="entregas"
    )
    ...
    feedback_professor = models.TextField(
        blank=True, help_text="Feedback geral da correção"
    )
...
class RespostaAluno(models.Model):
    ...
    entrega = models.ForeignKey(
        EntregaAtividade, on_delete=models.CASCADE, related_name="respostas"
    )
    ...
    texto_resposta = models.TextField(
        blank=True, null=True, help_text="Para questões discursivas"
    )
    comentario_professor = models.TextField(
        blank=True, help_text="Correção específica desta questão"
    )
    pontos_atribuidos = models.DecimalField(
...
class PlanejamentoAula(models.Model):
    ...
    disciplina = models.ForeignKey(
        Disciplina, on_delete=models.CASCADE, related_name="planejamentos_aula"
    )
    turma = models.ForeignKey(
        Turma, on_delete=models.CASCADE, related_name="planejamentos_aula"
    )
    data_aula = models.DateField(help_text="Data efetiva da aula")
    horario_aula = models.CharField(
        max_length=50, help_text="Horário conforme a Grade (Ex: 07:00 - 07:50)"
    )

    conteudo = models.TextField(
        help_text="Descrição do plano de aula ou conteúdo abordado (Markdown opcional)"
    )
    arquivos_apoio = models.FileField(
    ...
    status = models.CharField(
        max_length=15, choices=STATUS_AULA_CHOICES, default="NORMAL"
    )
...
class MaterialDidatico(models.Model):
    ...
    url = models.URLField(
        blank=True, null=True, help_text="Link externo (YouTube, site, etc.)"
    )
