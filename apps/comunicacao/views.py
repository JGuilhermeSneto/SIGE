...
def listar_comunicados(request):
    ...
    return render(
        request,
        "comunicacao/listar_comunicados.html",
        {
...
def cadastrar_editar_comunicado(request, pk=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not pk:
                try:
                    novo_comunicado.autor = (
                        request.user.gestor
                    )  # ✅ acessa o Gestor pelo related_name
                except Exception:
                    messages.error(
                        request, "Seu usuário não está vinculado a um Gestor."
                    )
    ...
    return render(
        request,
        "comunicacao/form_comunicado.html",
        {
            "form": form,
            "editando": bool(pk),
            "nome_exibicao": get_nome_exibicao(request.user),
