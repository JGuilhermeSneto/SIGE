...
class SaudeViewsTest(TestCase):
    def setUp(self):
        ...
        self.aluno = Aluno.objects.create(user=self.aluno_user, nome_completo="Aluno 1", cpf="2", data_nascimento="2010-01-01", turma=self.turma)
    ...
    def test_visualizar_saude_aluno_proprio(self):
        ...
        response = self.client.get(reverse('visualizar_saude_aluno', args=[self.aluno.id]))
    ...
    def test_editar_ficha_medica_gestor(self):
        ...
        response = self.client.post(reverse('editar_ficha_medica', args=[self.aluno.id]), data)
