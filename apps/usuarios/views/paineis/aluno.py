...
def perfil_aluno(request):
    """Página de perfil completo do aluno com histórico acadêmico."""
    aluno = getattr(request.user, "aluno", None)
    if not aluno:
        return render(request, "core/sem_perfil.html")
    ...
    for disciplina in disciplinas:
        ...
        frequencias = disciplina.frequencias_aluno_prefetched
        ...
        disciplinas_com_notas.append({
            ...
            "faltas": faltas,
            "presencas": presencas,
            "percentual_faltas": round(percentual_faltas, 1),
    ...
    percentual_frequencia = 100 - ((faltas_gerais / total_aulas_gerais * 100) if total_aulas_gerais > 0 else 0)

    if total_disciplinas_com_media > 0 or total_aulas_gerais > 0:
        sit_dict = _calcular_situacao_nota(media_geral, percentual_frequencia)
    else:
        sit_dict = {"texto": "Sem dados", "classe": ""}

    responsaveis = aluno.responsaveis.all()

    return render(request, "aluno/perfil_aluno.html", {
        "aluno": aluno,
        "foto_perfil_url": get_foto_perfil(request.user),
        "nome_exibicao": get_nome_exibicao(request.user),
        "disciplinas_com_notas": disciplinas_com_notas,
        ...
        "total_aulas_gerais": total_aulas_gerais,
