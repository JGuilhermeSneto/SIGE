...
def listar_faturas(request):
    ...
    paginator = Paginator(
        faturas.select_related("aluno").prefetch_related("pagamentos"), 20
    )  # 20 por página
    ...
    base_qs = (
        Fatura.objects.filter(aluno=usuario.aluno)
        if hasattr(usuario, "aluno")
...
def painel_financeiro(request):
    ...
    if not (hasattr(request.user, "gestor") or request.user.is_superuser):
        ...
        return HttpResponseForbidden(
            "Acesso negado. Apenas gestores podem visualizar o painel financeiro."
        )
    ...
    custo_folha = (
        folha_atual.aggregate(total=Sum("salario_base") + Sum("bonus"))["total"] or 0
    )
    status_folha = (
        "PAGA"
        if folha_atual.exists() and folha_atual.filter(pago=False).count() == 0
        else "PENDENTE"
    )
    ...
    despesas_cat = (
        Lancamento.objects.filter(tipo="SAIDA")
    ...
    ultimos_lancamentos = Lancamento.objects.select_related(
        "categoria", "autorizado_por"
    ...
    context = {
        ...
        "faturas_criticas": faturas_atrasadas.select_related("aluno").order_by(
            "data_vencimento"
        )[:5],
    }
...
def criar_lancamento(request):
    ...
    if request.method == "POST":
        ...
        return redirect(
            "financeiro:painel_financeiro"
            if tipo == "ENTRADA"
            else "financeiro:gestao_despesas"
        )
...
def notificar_fatura(request, fatura_id):
    ...
    if fatura.status == "PAGO":
        messages.warning(
            request, f"A fatura #{fatura.id} já está paga e não precisa de notificação."
        )
        return redirect("financeiro:detalhes_fatura", fatura_id=fatura.id)
