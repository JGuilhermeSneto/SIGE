...
class AuxiliaresTest(TestCase):
    ...
    def test_gerar_calendario_padrao(self):
        ...
        # cal é uma lista de dicionários
        self.assertIsInstance(cal, list)
        self.assertGreater(len(cal), 28)
        self.assertTrue(any(c["vazio"] is False and c["numero"] == 1 for c in cal))

    def test_gerar_calendario_parametros(self):
        cal = gerar_calendario(ano=2025, mes=6)
        # 1 de Junho de 2025 foi Domingo. Com firstweekday=6, deve ser o primeiro dia.
        # Mas vamos apenas verificar se o mês/ano injetado produziu células.
        self.assertTrue(any(c["numero"] == 1 for c in cal))
