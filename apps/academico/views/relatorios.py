...
def exportar_historico_pdf(request, aluno_id):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="historico_{aluno.cpf}.pdf"'
    )
    ...
    p.drawString(
        10 * cm,
        height - 4.7 * cm,
        f"DATA NASC.: {aluno.data_nascimento.strftime('%d/%m/%Y') if aluno.data_nascimento else 'N/A'}",
    )
    p.drawString(
        1 * cm,
        height - 5.2 * cm,
        f"TURMA ATUAL: {aluno.turma.nome} ({aluno.turma.get_turno_display()})",
    )
    ...
    for item in historico:
        ...
        p.drawString(
            x_offsets[2], y, f"{item['n1']:.1f}" if item["n1"] is not None else "-"
        )
        p.drawString(
            x_offsets[3], y, f"{item['n2']:.1f}" if item["n2"] is not None else "-"
        )
        p.drawString(
            x_offsets[4], y, f"{item['n3']:.1f}" if item["n3"] is not None else "-"
        )
        p.drawString(
            x_offsets[5], y, f"{item['n4']:.1f}" if item["n4"] is not None else "-"
        )
